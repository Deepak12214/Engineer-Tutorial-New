...
import { Plus, Search, Edit2, Trash2, Eye, FileText, BookOpen, Lock, Globe, Loader2 } from 'lucide-react'
...
export default function AdminContentPage() {
  ...
  useEffect(() => {
    async function load() {
      setLoading(true)
      try {
        const [topicsRes, blogsRes] = await Promise.all([
          fetch('/api/admin/topics'),
          fetch('/api/admin/blogs'),
        ])
        const topics = topicsRes.ok ? (await topicsRes.json()).map((t: any) => ({ ...t, contentType: 'topic' })) : []
        const blogs = blogsRes.ok ? (await blogsRes.json()).map((b: any) => ({ ...b, contentType: 'blog' })) : []
        setItems([...topics, ...blogs].sort((a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime()))
      } catch {}
      setLoading(false)
    }
    load()
  ...
  async function deleteItem(item: any) {
    ...
    const endpoint = item.contentType === 'topic' ? `/api/admin/topics/${item._id}` : `/api/admin/blogs/${item._id}`
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-16 text-slate-400">
            ...
            <p className="text-sm">{items.length === 0 ? 'No content yet. Create your first piece!' : 'No results match your filters.'}</p>
          </div>
        ) : (
                ...
                {filtered.map((item, i) => (
                  <tr key={item._id}
                    ...
                    <td className="px-4 py-3 hidden sm:table-cell">
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium capitalize ${item.contentType === 'topic' ? 'bg-blue-50 text-blue-600 dark:bg-blue-900/20 dark:text-blue-400' : 'bg-purple-50 text-purple-600 dark:bg-purple-900/20 dark:text-purple-400'}`}>
                        {item.contentType}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <span className={`text-xs px-2.5 py-1 rounded-full font-medium flex items-center gap-1 w-fit
                        ${item.isPublished ? 'bg-green-50 text-green-700 dark:bg-green-900/20 dark:text-green-400' : 'bg-slate-100 text-slate-600 dark:bg-slate-700 dark:text-slate-400'}`}>
                        {item.isPublished ? <><Globe className="w-3 h-3" /> Published</> : 'Draft'}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-slate-400 text-xs hidden lg:table-cell">
                      {new Date(item.updatedAt).toLocaleDateString('en-IN', { month: 'short', day: 'numeric', year: 'numeric' })}
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-1">
                        {item.isPublished && item.contentType === 'topic' && item.slug && (
                          <Link href={`/learn/${item.courseId?.slug ?? item.courseId}/${item.sectionId}/${item.slug}`}
                            target="_blank"
                            className="p-1.5 text-slate-400 hover:text-accent hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg" title="Preview">
                            <Eye className="w-4 h-4" />
                          </Link>
                        )}
                        {item.isPublished && item.contentType === 'blog' && item.slug && (
                        ...
                        <Link href={`/admin/content/${item._id}/edit`}
                        ...
                        <button onClick={() => deleteItem(item)}
                          className="p-1.5 text-slate-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg" title="Delete">
