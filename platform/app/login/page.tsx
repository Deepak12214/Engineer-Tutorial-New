...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex">
      {/* Left panel — Branding */}
      <div className="hidden lg:flex flex-col w-[45%] bg-gradient-to-br from-blue-600 to-indigo-700 p-12 text-white relative overflow-hidden">
        ...
        <div className="flex-1 flex flex-col justify-center z-10 relative">
          <h2 className="text-4xl font-extrabold mb-6 leading-tight">
            Learn from the<br />best engineers
          </h2>
          <div className="space-y-4">
            {['System Design from scratch to staff-level', 'Apache Kafka real-time pipelines', 'AI assistant while you learn', 'Built-in code editor & playground'].map(item => (
        ...
        <p className="text-blue-200 text-xs z-10 relative">Trusted by 12,000+ engineers worldwide</p>
      </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-8 shadow-sm">
            ...
            <p className="text-slate-500 text-sm mb-6">
              ...
              <span className="text-xs bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-400 px-2 py-0.5 rounded-full border border-amber-200 dark:border-amber-800">
                💡 Tip: use admin@x.com for admin access
              </span>
            </p>

            {/* Social login */}
            <div className="grid grid-cols-2 gap-3 mb-6">
              <button onClick={() => { login('user@google.com', '') }}
                className="flex items-center justify-center gap-2 py-2.5 border border-slate-200 dark:border-slate-700 rounded-xl text-sm font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800">
              ...
              <button onClick={() => { login('user@github.com', '') }}
                className="flex items-center justify-center gap-2 py-2.5 border border-slate-200 dark:border-slate-700 rounded-xl text-sm font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800">
            ...
            <p className="text-center text-sm text-slate-500 mt-6">
              ...
              <Link href="/register" className="text-accent font-semibold hover:underline">Create one free</Link>
            </p>
          </div>
        </div>
      </div>
