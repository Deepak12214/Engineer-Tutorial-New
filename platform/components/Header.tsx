...
import { BookOpen, Code2, FileText, Menu, Moon, Sun, X, ChevronDown, LayoutDashboard, LogOut, Settings, User, Zap, Search } from 'lucide-react'
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between h-14">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => setSearchOpen(true)}
              className="hidden sm:flex items-center gap-2 px-3 py-1.5 text-sm text-slate-400 bg-slate-100 dark:bg-slate-800 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors"
            >
              <Search className="w-3.5 h-3.5" />
              <span className="hidden lg:block">Search</span>
              <kbd className="hidden lg:block bg-white dark:bg-slate-700 text-slate-400 text-[10px] px-1.5 py-0.5 rounded border border-slate-200 dark:border-slate-600">⌘K</kbd>
            </button>
                ...
                {profileOpen && (
                  <div className="absolute right-0 top-full mt-2 w-56 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl shadow-xl p-2 z-50">
                    <div className="px-3 py-2 border-b border-slate-100 dark:border-slate-800 mb-1">
                      ...
                      <span className={`inline-block mt-1 text-xs px-2 py-0.5 rounded-full font-medium ${(user as any).subscriptionStatus === 'pro' ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/50 dark:text-blue-300' : 'bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400'}`}>
                        {(user as any).subscriptionStatus === 'pro' ? '⚡ Pro' : 'Free'}
                      </span>
                    </div>
                    ...
                    {(user as any).subscriptionStatus !== 'pro' && (
        ...
        {menuOpen && (
          <div className="md:hidden border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 px-4 py-4 space-y-2">
            <button onClick={() => { setSearchOpen(true); setMenuOpen(false) }}
              className="flex items-center gap-2 w-full p-2 text-sm text-slate-500 bg-slate-100 dark:bg-slate-800 rounded-lg mb-2">
              <Search className="w-4 h-4" /> Search courses, topics, blogs...
            </button>
            {courses.map(c => (
